...
async function openSpinCounter() {
    ...
    console.log("SpinWheelDocID", spinWheelDOCId);
    const config2 = {
        headers: {
            "ngrok-skip-browser-warning": "false",
        },
...
async function generateDiscountCode(priceRuleID, value) {
    ...
    // const addEmail = await axios.get(` https://spinwheelapp.herokuapp.com/`, config);
...
async function getPriceRule(getOfferText, getValue) {
    ...
    console.log("allPriceRules", allPriceRules);
...
console.log("newCode :-->", newCode);
    ...
    console.log("saveDiscountCodeToDB :-->", saveDiscountCodeToDB);
}
...
async function getToken(){
    ...
    const token = await axios.get(` https://spinwheelapp.herokuapp.com/api/token`, config);
    console.log("token :-->", token.data);

    if (token.data.getToken === null) {
